...
{
    public class GetFlightsFromToRequest : IRequest<GetFlightsFromToResponse>
    {
    ...
    public class GetFlightsFromToResponse 
    {
        public class Flight
        {
            ...
            public static Flight MapFromFlightModel(Domain.Models.Flight flight)
            {
                return new()
                {
                    Id = flight.Id,
    ...
    public class GetFlightsFromToCommandHandle : IRequestHandler<GetFlightsFromToRequest, GetFlightsFromToResponse>
    {
        private readonly IFlightRepository flightRepository;

        public GetFlightsFromToCommandHandle(IFlightRepository flightRepository)
        {
            this.flightRepository = flightRepository;
        }

        public async Task<GetFlightsFromToResponse> Handle(GetFlightsFromToRequest request, CancellationToken cancellationToken)
        {
            var flights = await flightRepository.GetFlightsAsync(request.From, request.To, request.DepartureTime, cancellationToken);
            return new GetFlightsFromToResponse
            {
                Flights = flights.Select(GetFlightsFromToResponse.Flight.MapFromFlightModel).ToList()
            };
        }
    }
}
